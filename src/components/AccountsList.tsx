  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
        ...
        <span className="text-cleanup-text-muted text-xs w-5 sm:w-6 flex-shrink-0">{index + 1}.</span>
        ...
        <a
          ...
          className="font-mono text-xs sm:text-sm text-cleanup-text-secondary hover:text-cleanup-primary transition-colors truncate"
        ...
        <button
          onClick={handleCopy}
          className="hidden sm:block text-cleanup-text-muted hover:text-cleanup-text-secondary text-xs transition-colors flex-shrink-0"
        ...
        {account.isPumpToken ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-shrink-0 flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 bg-cleanup-dark rounded-full hover:bg-cleanup-hover transition-colors border border-cleanup-border"
            onClick={(e) => e.stopPropagation()}
          ...
          >
            <svg className="w-[10px] h-[10px] sm:w-[12px] sm:h-[12px]" viewBox="0 0 252 300" fill="white" xmlns="http://www.w3.org/2000/svg">
      ...
      <span className={`font-mono text-xs sm:text-sm whitespace-nowrap flex-shrink-0 ml-2 ${isSelected ? 'text-cleanup-secondary' : 'text-cleanup-text-muted'}`}>
